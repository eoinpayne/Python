...
def getDistanceList (codes,city1,city2, lats, longs):  ## Q3 Getting distance between 2 airports from lists.
    listIndex1 = codes.index(city1)  ##determiing the placeholder value of "city1" (our user defined location) in the list. Will return 0,1,2,3 etc. this will be used below to pick points in other lists to populate variables.
    listIndex2 = codes.index(city2)

        ### THIS ASSIGNS THE VARIABLES WITH THE VALUE OF LIST FROM ITS PLACE VALUE.
    lat1 = lats[listIndex1]  ## here we assingning lat1 as the first position of the list it is being called from. i.e the place the loop is currently at [i], it will take from that position in list
    long1 = longs [listIndex1]
    lat2 = lats [listIndex2]
    long2 = longs[listIndex2]

    distance_airports_from_list = calcDistance(lat1,lat2,long1,long2)
    print("List distnace From", codes[listIndex1], "To", codes[listIndex2], "is", ("%.2f" % distance_airports_from_list), "km")
